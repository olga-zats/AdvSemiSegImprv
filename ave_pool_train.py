...
from model.pool_discriminator import FCDiscriminator
...
def get_arguments():
    ...
    parser.add_argument("--kernel-size", type=str, default=INPUT_SIZE,
                        help="Kernel size for average pooling operation")
...
def main():
    ...
    kh, kw = map(int, args.kernel_size.split(','))
    kernel_size = (kh, kw)
    print(kernel_size)
    ...
    # init D
    model_D = FCDiscriminator(num_classes=args.num_classes, kernel_size=kernel_size)
    ...
    for i_iter in range(args.num_steps):
        ...
        print('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_adv_p = {3:.3f}, loss_D = {4:.3f}, loss_semi_adv = {5:.3f}'.format(i_iter, args.num_steps, loss_seg_value, loss_adv_pred_value, loss_D_value, loss_semi_adv_value))

        if i_iter >= args.num_steps-1:
            print ('save model ...')
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'VOC_'+str(args.num_steps) + '_' + str(kh) + '_' + str(args.random_seed) + '.pth'))
            torch.save(model_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_'+str(args.num_steps) + '_' + str(kh) + '_' + str(args.random_seed) +  '_D.pth'))
            break

        if i_iter % args.save_pred_every == 0 and i_iter!=0:
            print ('taking snapshot ...')
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'VOC_'+str(i_iter) + '_' + str(kh) + '_' + str(args.random_seed) + '.pth'))
            torch.save(model_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_'+str(i_iter) + '_' + str(kh) + '_' + str(args.random_seed) +  '_D.pth'))
