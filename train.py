...
def main():
    ...
    if args.restore_from[:4] == 'http' :
       ...
       saved_state_dict = torch.load(args.restore_from, map_location={'cuda:1' : 'cuda:0'})
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            if (args.lambda_semi > 0 or args.lambda_semi_adv > 0 ) and i_iter >= args.semi_start_adv :
                ...
                if args.lambda_semi <= 0 or i_iter < args.semi_start:
                    loss_semi_adv.backward()
                    last_layer_grad_1 = model.layer5.conv2d_list[-1].weight.grad
                    loss_semi_value = 0
                else:
                    # produce ignore mask
            ...
            D_out_sigmoid = F.sigmoid(D_out).data.cpu().numpy() #.squeeze(axis=1)
            conf =  D_out_sigmoid
            conf_pred = np.count_nonzero(conf <= 0.5)
        ...
        if i_iter >= args.num_steps-1:
            print 'save model ...'
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'VOC_' + str(args.random_seed) + '_' +  str(args.num_steps)+'.pth'))
            torch.save(model_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_' + str(args.random_seed) + '_' +str(args.num_steps)+'_D.pth'))
            break

        if i_iter % args.save_pred_every == 0 and i_iter!=0:
            print 'taking snapshot ...'
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'VOC_' + str(args.random_seed) + '_' +str(i_iter)+'.pth'))
