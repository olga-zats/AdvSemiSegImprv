...
class VOCClsDataSet(data.Dataset):
    ...
    def one_hot_cls_label(self, label, num_classes=21):
        cls_label = np.zeros((num_classes - 1, 1, 1), dtype=np.float32)
        clsIDs = np.unique(label).astype(np.uint8).tolist()
        for clsID in clsIDs:
            if clsID in [0, 255]:
                continue
            cls_label[clsID - 1, 0, 0] = 1.0
        return cls_label
    ...
    def __getitem__(self, index):
        ...
        label = cv2.imread(datafiles["label"], cv2.IMREAD_GRAYSCALE)
        #print(label.shape)
        label = self.one_hot_cls_label(label)
        #print('successfully called one hot cls label') 
        size = image.shape
        name = datafiles["name"]
        if self.scale:
            image = self.generate_scale(image)
        image = np.asarray(image, np.float32)
        image -= self.mean
