...
def get_arguments():
    ...
    parser.add_argument("--interp-alpha", type=float, default=1.0)
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            if args.D_remain and (args.lambda_semi > 0 or args.lambda_semi_adv > 0):
            ...
            images, labels_gt, _, _ = batch
            with torch.no_grad():
                images = images.cuda(args.gpu)
                pred_l = F.softmax(interp(model(images)))

            D_gt_v = Variable(one_hot(labels_gt)).cuda(args.gpu)           
            smoothed_gt = args.interp_alpha * D_gt_v + (1.0 - args.interp_alpha) * pred_l
            ignore_mask_gt = (labels_gt.numpy() == 255)

            D_out = interp(model_D(smoothed_gt))
        ...
        if i_iter >= args.num_steps-1:
            print 'save model ...'
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'VOC_'  +  str(args.num_steps)+ '_' + str(args.random_seed) + '_' + str(args.interp_alpha) + '.pth'))
            torch.save(model_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_'  +str(args.num_steps)+ '_' + str(args.random_seed) + '_' + str(args.interp_alpha) + '_D.pth'))
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter!=0:
            ...
            torch.save(model_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_' + str(args.random_seed) + '_' +str(i_iter)+'_D.pth'))
