...
from model.deeplab_class import Res_Deeplab as Res_Deeplab_class
...
def get_arguments():
    ...
    parser.add_argument("--latent-vars", type=int, default=5,
                        help="Number of latent vars excluding background.")
    parser.add_argument("--restore-from", type=str, default=RESTORE_FROM,
                        help="Where restore model parameters from.")
    parser.add_argument("--restore-from-classifier", type=str, default=RESTORE_FROM,
                        help="Where restore model parameters from.")
    parser.add_argument("--sigmoid-threshold", type=float, default=0.5,
                        help="Where restore model parameters from.")
...
def get_iou(data_list, class_num, save_path=None):
    ...
    for i, iou in enumerate(j_list):
        print('class {:2d} {:12} IoU {:.2f}'.format(i, classes[i], j_list[i]))
...
def main():
    ...
    model_cls = Res_Deeplab_class(num_classes=args.num_classes, mode=6, latent_vars=args.latent_vars)
    ...
    combo_matrix = np.zeros((args.num_classes, args.latent_vars + 1), dtype=np.float32)
    for index, batch in enumerate(testloader):
        ...
        cls_pred = F.sigmoid(model_cls(Variable(image, volatile=True).cuda(gpu0)))
    ...
    filename = os.path.join(args.save_dir, args.restore_from.split('/')[-1][:-4] + '_with_classifier_result.txt')
